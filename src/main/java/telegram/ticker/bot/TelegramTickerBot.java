...
public class TelegramTickerBot extends TelegramLongPollingBot {
    ...
    private String formatIndicatorsTable(String ticker, FIIData data) {
        ...
        table.append("```\n");
        table.append("┌──────────────┬────────────┐\n");
        table.append("│ Div. Yield   │ %10s │%n".formatted(data.getDividendYield()));
        table.append("│ P/VP         │ %10s │%n".formatted(data.getPVp()));
        table.append("│ Cotistas     │ %10s │%n".formatted(data.getCotistas()));
        table.append("│ Imóveis      │ %10s │%n".formatted(data.getImoveis()));
        table.append("└──────────────┴────────────┘\n");
        table.append("```\n");
    ...
    private String formatStockTable(String ticker, StockData data) {
        ...
        table.append("```\n");
        table.append("┌────────────────┬────────────┐\n");
        table.append("│ P/L            │ %10s │%n".formatted(data.getPl()));
        table.append("│ Div. Yield     │ %10s │%n".formatted(data.getDividendYield()));
        table.append("│ P/VP           │ %10s │%n".formatted(data.getPVp()));
        table.append("│ ROE            │ %10s │%n".formatted(data.getRoe()));
        table.append("│ Dív.Líq/EBITDA │ %10s │%n".formatted(data.getDividaLiquidaEbitda()));
        table.append("│ Marg. EBITDA   │ %10s │%n".formatted(data.getMargemEbitda()));
        table.append("│ LPA            │ %10s │%n".formatted(data.getLpa()));
        table.append("│ Cresc.Rec 5a   │ %10s │%n".formatted(data.getCrescimentoReceita5a()));
        table.append("└────────────────┴────────────┘\n");
        table.append("```\n");
        return table.toString();
    }
    ...
    private String formatUSStockTable(String ticker, USStockData data) {
        ...
        table.append("├────────────────┼────────────┤\n");
        table.append("│ P/E            │ %10s │%n".formatted(data.getPe()));
        table.append("│ P/S            │ %10s │%n".formatted(data.getPs()));
        table.append("│ EV/EBITDA      │ %10s │%n".formatted(data.getEvEbitda()));
        table.append("│ P/BV           │ %10s │%n".formatted(data.getPbv()));
        table.append("│ PEG            │ %10s │%n".formatted(data.getPeg()));
        table.append("│ FCF Yield      │ %10s │%n".formatted(data.getFcfYield()));
        table.append("├────────────────┼────────────┤\n");
        table.append("│  FINANCIALS    │            │\n");
        table.append("├────────────────┼────────────┤\n");
        table.append("│ ROE            │ %10s │%n".formatted(data.getRoe()));
        table.append("│ Marg. EBITDA   │ %10s │%n".formatted(data.getMargemEbitda()));
        table.append("│ Debt/EBITDA    │ %10s │%n".formatted(data.getDebtEbitda()));
        table.append("│ Current Ratio  │ %10s │%n".formatted(data.getCurrentRatio()));
        table.append("│ Div. Yield     │ %10s │%n".formatted(data.getDividendYield()));
        table.append("│ Payout Ratio   │ %10s │%n".formatted(data.getPayoutRatio()));
        table.append("└────────────────┴────────────┘\n");
        table.append("```\n");
        return table.toString();
    }
    ...
    private String formatCryptoTable(String symbol, CryptoData data) {
        ...
        table.append("```\n");
        table.append("┌──────────────┬────────────────┐\n");
        table.append("│ Preço        │ %14s │%n".formatted(data.getPrice()));
        table.append("│ Market Cap   │ %14s │%n".formatted(data.getMarketCap()));
        table.append("│ Ranking      │ %14s │%n".formatted("#" + data.getRank()));
        table.append("└──────────────┴────────────────┘\n");
        table.append("```\n");
        return table.toString();
    }
